...
pub(crate) fn install_hook_assets(project_root: &Path, host: &str) -> Result<serde_json::Value> {
    ...
    write_default_guidance(&guidance_path)?;
...
fn write_default_guidance(guidance_path: &Path) -> Result<()> {
    let should_write = match fs::read_to_string(guidance_path) {
        Ok(existing) => {
            let existing = existing.trim();
            existing == DEFAULT_GROUNDING_GUIDANCE.trim()
                || existing == LEGACY_DEFAULT_GROUNDING_GUIDANCE.trim()
        }
...
const LEGACY_DEFAULT_GROUNDING_GUIDANCE: &str = r#"# ctx Grounding Guidance
